...
int	grav_anom_type, 		/*Gravity anomaly type [m]: 0 means no gravity calculation; 1 means Bouguer; 2 means free-Air*/
	isost_model;			/*Type of isostasy / lithospheric plate: 
						0 means no isostasy
						1 means pure elastic
						2 means viscoelastic
						3 means elastic-plastic oceanic
						4 means elastic-plastic continental always decoupled
						5 means elastic-plastic continental never decoupled
						6 means elastic-plastic continental automatically decoupled*/

float	Te_default, 			/*Default elastic plate thicknes [m]*/
...
char	projectname[MAXLONFICH], 	/*The project name is also the root of current model file names*/
	title[MAXLONLINE];		/*Title with main current model characteristics*/


BOOL	switch_sea, 			/*SI if sea presence has been required*/
